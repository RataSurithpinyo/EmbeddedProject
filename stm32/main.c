...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  sprintf(uartBuf, "Distance (cm) = %d.%d  Light: %d\r\n", b, r, adcval);
	  //HAL_UART_Transmit(&huart2, (uint8_t *)uartBuf, strlen(uartBuf), 100);
	  //HAL_UART_Transmit(&huart1, uartBuf, sizeof(uartBuf), 100);
	  //HAL_UART_Transmit(&huart1, te, sizeof(te), 1000);
	  char tmpBuf[5];
	  tmpBuf[0] = 255;
	  tmpBuf[1] = distance/10;
	  tmpBuf[2] = (distance*10 - tmpBuf[1]*100)/1;
	  tmpBuf[3] = adcval/100;
	  tmpBuf[4] = adcval - tmpBuf[3];
	  HAL_UART_Transmit(&huart1, tmpBuf, 5, 100);
	  HAL_Delay(1000);
  }
